...
書き出しません。隠しファイル・隠しディレクトリの一覧での扱いは従来どおりです。
`.gitignore` 等も、ファイルを明示選択した場合はテキストとして変換できます。

ソースの読み取りに不要な `.jsonl`・`.svg`・`.trx`・`.lock`・`.sha256`・`.sha512`・
`.metadata`・`.map`・`.patch`・`.diff` は内容を取り込みません。
退避用の `.bak`・`.bak-…`・`.bak_…` も除外します。`.log`・`.csv`・`.tsv` は従来どおり対象です。
...
隠しファイル・隠しディレクトリの探索範囲と、PDF / Office の既存のサイズ上限は変えていません。
